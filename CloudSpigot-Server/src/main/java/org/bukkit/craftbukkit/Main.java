	...
	public static void main(String[] args) {
		...
		OptionParser parser = new OptionParser() {
			{
				...
				// Paper start
				acceptsAll(asList("server-name"), "Name of the server").withRequiredArg().ofType(String.class)
						.defaultsTo("Unknown Server").describedAs("Name");
				// Paper end

				// CloudSpigot start
				acceptsAll(asList("cloudspigot", "cloudspigot-settings"), "File for cloudspigot settings")
						.withRequiredArg().ofType(File.class).defaultsTo(new File("cloudspigot.yml"))
		...
		if ((options == null) || (options.has("?"))) {
			...
			try {
				...
				if (false && Main.class.getPackage().getImplementationVendor() != null
						&& System.getProperty("IReallyKnowWhatIAmDoingISwear") == null) {
					Date buildDate = new SimpleDateFormat("yyyyMMdd-HHmm")
							.parse(Main.class.getPackage().getImplementationVendor());
					...
					if (buildDate.before(deadline.getTime())) {
						...
						System.err.println(
								"*** Please download a new build as per instructions from https://paperci.emc.gs/ ***"); // Paper
