...
public class PlayerConnection implements PacketListenerPlayIn, ITickable {
	...
	private int e;
	private long f = getCurrentMillis();
	...
	private long getLastPing() {
		return this.f;
	} // Paper - OBFHELPER - set ping to delay initial
	...
	private final static HashSet<Integer> invalidItems = new HashSet<Integer>(
			java.util.Arrays.asList(8, 9, 10, 11, 26, 34, 36, 43, 51, 55, 59, 62, 63, 64, 68, 71, 74, 75, 83, 90, 92,
					93, 94, 104, 105, 115, 117, 118, 119, 125, 127, 132, 140, 141, 142, 144)); // TODO: Check after
																								// every update.
	// CraftBukkit end
	...
	public void e() {
		...
		if (this.player.J() > 0L && this.minecraftServer.getIdleTimeout() > 0 && MinecraftServer.aw()
				- this.player.J() > (long) (this.minecraftServer.getIdleTimeout() * 1000 * 60)) {
		...
		if (b(packetplayinvehiclemove)) {
			...
			if (entity != this.player && entity.bE() == this.player && entity == this.r) {
				...
				if (player.world.paperConfig.preventMovingIntoUnloadedChunks
						&& !worldserver.isChunkLoaded((int) Math.floor(packetplayinvehiclemove.getX()) >> 4,
								(int) Math.floor(packetplayinvehiclemove.getZ()) >> 4, false)) {
				...
				if (d10 - d9 > Math.max(100.0D,
						...
						&& (!this.minecraftServer.R() || !this.minecraftServer.Q().equals(entity.getName()))) { // Spigot
					// CraftBukkit end
					PlayerConnection.LOGGER.warn("{} (vehicle of {}) moved too quickly! {},{},{}", entity.getName(),
				...
				this.D = d11 >= -0.03125D && !this.minecraftServer.getAllowFlight()
						&& !worldserver.c(entity.getBoundingBox().g(0.0625D).b(0.0D, -0.55D, 0.0D));
				this.v = entity.locX;
				this.w = entity.locY;
				this.x = entity.locZ;
			}
			...
			if (!this.player.viewingCredits && !this.player.isFrozen()) { // CraftBukkit
				...
				if (this.teleportPos != null) {
					...
					if (this.player.isPassenger()) {
						...
						double d8 = d5 - this.m;
						double d9 = d6 - this.n;
						double d10 = this.player.motX * this.player.motX + this.player.motY * this.player.motY
						...
						if (this.player.isSleeping()) {
							...
							if (player.world.paperConfig.preventMovingIntoUnloadedChunks
									...
									&& !worldserver.isChunkLoaded((int) Math.floor(toX) >> 4,
											(int) Math.floor(toZ) >> 4, false)) {
							...
							if (!this.player.L()
									&& (!this.player.x().getGameRules().getBoolean("disableElytraMovementCheck")
											|| !this.player.cP())) {
								...
								if (d11 - d10 > Math.max(f2,
										...
										&& (!this.minecraftServer.R()
												|| !this.minecraftServer.Q().equals(this.player.getName()))) { // Spigot
									// CraftBukkit end
									PlayerConnection.LOGGER.warn("{} moved too quickly! {},{},{}",
											this.player.getName(), Double.valueOf(d7), Double.valueOf(d8),
											Double.valueOf(d9));
							...
							boolean flag = worldserver
									.getCubes(this.player, this.player.getBoundingBox().shrink(0.0625D)).isEmpty();
							...
							if (!this.player.L() && d11 > org.spigotmc.SpigotConfig.movedWronglyThreshold
							...
							this.B &= !this.player.hasEffect(MobEffects.LEVITATION) && !this.player.cP()
									&& !worldserver.c(this.player.getBoundingBox().g(0.0625D).b(0.0D, -0.55D, 0.0D));
							this.player.onGround = packetplayinflying.a();
							this.minecraftServer.getPlayerList().d(this.player);
	...
	public void a(PacketPlayInBlockDig packetplayinblockdig) {
		...
		case SWAP_HELD_ITEMS:
			if (!this.player.isSpectator()) {
				...
				// CraftBukkit start
				PlayerSwapHandItemsEvent swapItemsEvent = new PlayerSwapHandItemsEvent(getPlayer(),
						CraftItemStack.asBukkitCopy(itemstack),
						CraftItemStack.asBukkitCopy(this.player.b(EnumHand.MAIN_HAND)));
		...
		case STOP_DESTROY_BLOCK:
			...
			if (d3 > 36.0D) {
				if (worldserver.isChunkLoaded(blockposition.getX() >> 4, blockposition.getZ() >> 4, true)) // Paper -
																											// Fix block
																											// break
																											// desync -
																											// Don't
																											...
																											// unloaded
																											// chunks
					this.sendPacket(new PacketPlayOutBlockChange(worldserver, blockposition)); // Paper - Fix block
																								// break desync
				return;
				...
				if (packetplayinblockdig.c() == PacketPlayInBlockDig.EnumPlayerDigType.START_DESTROY_BLOCK) {
					if (!this.minecraftServer.a(worldserver, blockposition, this.player)
						...
						CraftEventFactory.callPlayerInteractEvent(this.player, Action.LEFT_CLICK_BLOCK, blockposition,
								packetplayinblockdig.b(), this.player.inventory.getItemInHand(), EnumHand.MAIN_HAND);
	...
	public void a(PacketPlayInUseItem packetplayinuseitem) {
		...
		this.player.resetIdleTimer();
		if (blockposition.getY() >= this.minecraftServer.getMaxBuildHeight() - 1 && (enumdirection == EnumDirection.UP
			...
			this.player.playerConnection.sendPacket(new PacketPlayOutChat(chatmessage, ChatMessageType.GAME_INFO));
				...
				&& this.player.d((double) blockposition.getX() + 0.5D, (double) blockposition.getY() + 0.5D,
			...
			// CraftBukkit end
			this.player.playerInteractManager.a(this.player, worldserver, itemstack, enumhand, blockposition,
					enumdirection, packetplayinuseitem.d(), packetplayinuseitem.e(), packetplayinuseitem.f());
		}
			...
			if (movingobjectposition == null
				...
				if (player.playerInteractManager.firedInteract) {
					...
					org.bukkit.event.player.PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(
							player, Action.RIGHT_CLICK_BLOCK, movingobjectposition.a(), movingobjectposition.direction,
							itemstack, true, enumhand);
					cancelled = event.useItemInHand() == Event.Result.DENY;
				}
			}
	...
	public void a(PacketPlayInSpectate packetplayinspectate) {
		...
		if (this.player.isSpectator()) {
			...
			if (entity != null) {
				 ...
				 * (entity.world == this.player.world) {
				 * this.player.enderTeleportTo(entity.locX, entity.locY, entity.locZ); } else {
				 * WorldServer worldserver1 = this.player.x(); WorldServer worldserver2 =
	...
	public void a(PacketPlayInResourcePackStatus packetplayinresourcepackstatus) {
		...
		// Paper start
		// this.server.getPluginManager().callEvent(new
		// PlayerResourcePackStatusEvent(getPlayer(),
		// PlayerResourcePackStatusEvent.Status.values()[packetplayinresourcepackstatus.status.ordinal()]));
		final PlayerResourcePackStatusEvent.Status status = PlayerResourcePackStatusEvent.Status
	...
	public void a(IChatBaseComponent ichatbasecomponent) {
		...
		/*
		 * this.minecraftServer.aD(); ChatMessage chatmessage = new
	...
	public void sendPacket(final Packet<?> packet) {
		if (packet instanceof PacketPlayOutChat) {
			...
			if (entityhuman_enumchatvisibility == EntityHuman.EnumChatVisibility.HIDDEN
					&& packetplayoutchat.c() != ChatMessageType.GAME_INFO) {
				return;
			}
		...
		if (entity != null) {
			...
			if (this.player.h(entity) < d0) {
				...
				ItemStack itemInHand = this.player
						.b(packetplayinuseentity.b() == null ? EnumHand.MAIN_HAND : packetplayinuseentity.b()); // CraftBukkit
				...
				if (packetplayinuseentity.a() == PacketPlayInUseEntity.EnumEntityUseAction.INTERACT
					...
					if (packetplayinuseentity.a() == PacketPlayInUseEntity.EnumEntityUseAction.INTERACT) {
						event = new PlayerInteractEntityEvent((Player) this.getPlayer(), entity.getBukkitEntity(),
								(packetplayinuseentity.b() == EnumHand.OFF_HAND) ? EquipmentSlot.OFF_HAND
										: EquipmentSlot.HAND);
					} else {
					...
					if (event.isCancelled() || this.player.inventory.getItemInHand() == null
						...
						entity.tracker
								.broadcast(new PacketPlayOutEntityMetadata(entity.getId(), entity.datawatcher, true)); // Paper
																														// -
																														// update
																														// entity
																														// for
		...
		// Paper start - fire event
			...
			this.server.getPluginManager()
					.callEvent(new com.destroystokyo.paper.event.player.PlayerUseUnknownEntityEvent(this.getPlayer(),
							...
							packetplayinuseentity.b() == EnumHand.MAIN_HAND ? EquipmentSlot.HAND
	...
	public void a(PacketPlayInWindowClick packetplayinwindowclick) {
		...
		if (this.player.activeContainer.windowId == packetplayinwindowclick.a()
			...
			case THROW:
				...
				break;
			case QUICK_CRAFT:
				itemstack = this.player.activeContainer.a(packetplayinwindowclick.b(), packetplayinwindowclick.c(),
	...
	public void a(PacketPlayInAutoRecipe packetplayinautorecipe) {
		...
		this.player.resetIdleTimer();
		if (!this.player.isSpectator() && this.player.activeContainer.windowId == packetplayinautorecipe.a()
	...
	public void a(PacketPlayInSetCreativeSlot packetplayinsetcreativeslot) {
		...
		if (this.player.playerInteractManager.isCreative()) {
			...
			boolean flag2 = itemstack.isEmpty() || itemstack.getData() >= 0 && itemstack.getCount() <= 64
					&& !itemstack.isEmpty() && (!invalidItems.contains(Item.getId(itemstack.getItem()))
	...
	public void a(PacketPlayInUpdateSign packetplayinupdatesign) {
		...
		if (worldserver.isLoaded(blockposition)) {
			...
			if (!tileentitysign.a() || tileentitysign.signEditor == null
					|| !tileentitysign.signEditor.equals(this.player.getUniqueID())) { // Paper
				this.minecraftServer
			...
			for (int i = 0; i < astring.length; ++i) {
				...
				// normal
				if (astring[i].length() > TileEntitySign.MAX_SIGN_LINE_LENGTH
						&& TileEntitySign.MAX_SIGN_LINE_LENGTH > 0) {
					int offset = astring[i].codePoints().limit(TileEntitySign.MAX_SIGN_LINE_LENGTH)
	...
	public void a(PacketPlayInKeepAlive packetplayinkeepalive) {
		...
		// this.player.x()); // CraftBukkit // Paper - This shouldn't be on the main
		// thread
		if (this.g && packetplayinkeepalive.a() == this.h) {
			...
			PlayerConnection.LOGGER.warn(
					"{} sent an invalid keepalive! pending keepalive: {} got id: {} expected id: {}",
	...
	public void a(PacketPlayInTabComplete packet) {
		...
		event = new com.destroystokyo.paper.event.server.AsyncTabCompleteEvent(this.getPlayer(), completions, buffer,
				isCommand, blockpos != null ? MCUtil.toLocation(player.world, blockpos) : null);
		event.callEvent();
		...
		if (!event.isHandled()) {
			...
			this.player.playerConnection
					.sendPacket(new PacketPlayOutTabComplete(completions.toArray(new String[completions.size()])));
	...
	public void a(PacketPlayInCustomPayload packetplayincustompayload) {
		...
		if ("MC|BEdit".equals(s)) {
			...
			try {
				...
				IllegalPacketEvent.process(player.getBukkitEntity(), "InvalidBookEdit", "Invalid book data!",
						exception); // Paper
			}
		} else {
			String s1;
			...
			if ("MC|BSign".equals(s)) {
				...
				try {
					...
					IllegalPacketEvent.process(player.getBukkitEntity(), "InvalidBookSign", "Invalid book data!",
							exception1); // Paper
				}
				...
				try {
					...
					IllegalPacketEvent.process(player.getBukkitEntity(), "InvalidTrade", "Invalid trade data!",
							exception2); // Paper
				}
			} else {
				...
				if ("MC|AdvCmd".equals(s)) {
					...
					try {
						...
						BlockPosition blockposition = new BlockPosition(packetdataserializer.readInt(),
								packetdataserializer.readInt(), packetdataserializer.readInt());
						...
						TileEntityCommand.Type tileentitycommand_type = TileEntityCommand.Type
								.valueOf(packetdataserializer.e(16));
						...
						if (commandblocklistenerabstract1 != null) {
							...
							case SEQUENCE:
								...
								this.player.world.setTypeAndData(blockposition, iblockdata
										.set(BlockCommand.a, enumdirection).set(BlockCommand.b, Boolean.valueOf(flag2)),
										2);
								break;
							...
							if (!UtilColor.b(s3)) {
								this.player.sendMessage(
										new ChatMessage("advMode.setCommand.success", new Object[] { s3 }));
							}
						}
					...
					if ("MC|Beacon".equals(s)) {
						if (this.player.activeContainer instanceof ContainerBeacon) {
							try {
								...
								IllegalPacketEvent.process(player.getBukkitEntity(), "InvalidBeacon",
										"Invalid beacon data!", exception5); // Paper
							}
						}
						...
						try {
							BlockPosition blockposition1 = new BlockPosition(packetdataserializer.readInt(),
									packetdataserializer.readInt(), packetdataserializer.readInt());
							...
							if (tileentity instanceof TileEntityStructure) {
								...
								if (b1 == 2) {
									if (tileentitystructure.q()) {
										this.player
												.a((IChatBaseComponent) (new ChatMessage("structure_block.save_success",
														new Object[] { s7 })), false);
									} else {
										this.player
												.a((IChatBaseComponent) (new ChatMessage("structure_block.save_failure",
														new Object[] { s7 })), false);
									}
								} else if (b1 == 3) {
									if (!tileentitystructure.E()) {
										this.player.a(
												(IChatBaseComponent) (new ChatMessage("structure_block.load_not_found",
														new Object[] { s7 })),
												false);
									} else if (tileentitystructure.r()) {
										this.player
												.a((IChatBaseComponent) (new ChatMessage("structure_block.load_success",
														new Object[] { s7 })), false);
									} else {
										this.player
												.a((IChatBaseComponent) (new ChatMessage("structure_block.load_prepare",
														new Object[] { s7 })), false);
									}
								} else if (b1 == 4) {
									if (tileentitystructure.p()) {
										this.player
												.a((IChatBaseComponent) (new ChatMessage("structure_block.size_success",
														new Object[] { s7 })), false);
									} else {
										this.player
												.a((IChatBaseComponent) (new ChatMessage("structure_block.size_failure",
														new Object[0])), false);
									}
								}
						...
						try {
							...
							this.player.playerConnection
									.sendPacket(new PacketPlayOutSetSlot(-2, this.player.inventory.itemInHandIndex,
											this.player.inventory.getItem(this.player.inventory.itemInHandIndex)));
							this.player.playerConnection
									.sendPacket(new PacketPlayOutSetSlot(-2, k, this.player.inventory.getItem(k)));
							this.player.playerConnection
									.sendPacket(new PacketPlayOutHeldItemSlot(this.player.inventory.itemInHandIndex));
						} catch (Exception exception7) {
							IllegalPacketEvent.process(player.getBukkitEntity(), "InvalidPickItem", "Invalid PickItem",
									exception7); // Paper
						}
					}
					// CraftBukkit start
