...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
	...
	private final java.util.Queue<ServerCommand> serverCommandQueue = new java.util.concurrent.ConcurrentLinkedQueue<>(); // Paper
																															// -
																															// use
																															// a
																															// proper
	...
	public DedicatedServer(joptsimple.OptionSet options, DataConverterManager dataconvertermanager,
			...
			UserCache usercache) {
		super(options, Proxy.NO_PROXY, dataconvertermanager, yggdrasilauthenticationservice, minecraftsessionservice,
				gameprofilerepository, usercache);
	}
	...
	public boolean init() throws IOException { // CraftBukkit - decompile error
		Thread thread = new Thread("Server console handler") {
			public void run() {
				...
				/*
				 * jline.console.ConsoleReader bufferedreader = reader; // CraftBukkit end
				 * 
				 * String s;
				 * 
		...
		if (!this.r.a() && !eulaAgreed) { // Spigot
			...
			// com.destroystokyo.paper.VersionHistoryManager.INSTANCE.getClass(); // load
			// version history now // CloudSpigot
			// Paper end
			...
			DedicatedServer.LOGGER.info("Starting Minecraft server on {}:{}",
					this.getServerIp().isEmpty() ? "*" : this.getServerIp(), Integer.valueOf(this.P()));
			...
			if (!NameReferencingFileConverter.a(this.propertyManager)) {
				 ...
				 * this.worlds.get(0).getGameRules().set("announceAdvancements",
				 * this.propertyManager.getBoolean("announce-player-achievements", true) ?
				 * "true" : "false"); // CraftBukkit
				...
				if (this.propertyManager.getBoolean("enable-rcon", false)) {
					...
					this.remoteConsole = new org.bukkit.craftbukkit.command.CraftRemoteConsoleCommandSender(
							this.remoteControlCommandListener); // CraftBukkit
				}
	...
	public String aO() {
		if (this.propertyManager.a("resource-pack-hash")) {
			if (this.propertyManager.a("resource-pack-sha1")) {
				...
				this.propertyManager.getString("resource-pack-sha1",
	...
	public void a(MojangStatisticsGenerator mojangstatisticsgenerator) {
		 ...
		 * Boolean.valueOf(this.aQ().getHasWhitelist()));
		 * mojangstatisticsgenerator.a("whitelist_count",
		 * Integer.valueOf(this.aQ().getWhitelisted().length));
	...
	public String executeRemoteCommand(final String s) {
		...
		// Paper start
		if (s.toLowerCase().startsWith("timings")
				&& s.toLowerCase().matches("timings (report|paste|get|merged|seperate)")) {
