...
public class ContainerEnchantTable extends Container {
		...
		if (iinventory == this.enchantSlots) {
			...
			if (!itemstack.isEmpty()) { // CraftBukkit - relax condition
				if (!this.world.isClientSide) {
					...
					for (j = 0; j < 3; ++j) {
						org.bukkit.enchantments.Enchantment enchantment = (this.h[j] >= 0)
								? org.bukkit.enchantments.Enchantment.getById(this.h[j])
								: null;
						offers[j] = (enchantment != null) ? new EnchantmentOffer(enchantment, this.i[j], this.costs[j])
								: null;
					}
	...
	public boolean a(EntityHuman entityhuman, int i) {
		...
		if ((itemstack1.isEmpty() || itemstack1.getCount() < j) && !entityhuman.abilities.canInstantlyBuild) {
			return false;
		} else if (this.costs[i] > 0 && !itemstack.isEmpty()
			...
			if (!this.world.isClientSide) {
				...
				for (Object obj : list) {
					...
					enchants.put(org.bukkit.enchantments.Enchantment.getById(Enchantment.getId(instance.enchantment)),
							instance.level);
				}
				CraftItemStack item = CraftItemStack.asCraftMirror(itemstack);
				...
				for (Map.Entry<org.bukkit.enchantments.Enchantment, Integer> entry : event.getEnchantsToAdd()
					...
					try {
						if (flag) {
							...
							WeightedRandomEnchant weightedrandomenchant = new WeightedRandomEnchant(
									Enchantment.c(enchantId), entry.getValue());
				...
				this.world.a((EntityHuman) null, this.position, SoundEffects.aR, SoundCategory.BLOCKS, 1.0F,
	...
	public boolean canUse(EntityHuman entityhuman) {
		...
		return this.world.getType(this.position).getBlock() != Blocks.ENCHANTING_TABLE ? false
				: entityhuman.d((double) this.position.getX() + 0.5D, (double) this.position.getY() + 0.5D,
