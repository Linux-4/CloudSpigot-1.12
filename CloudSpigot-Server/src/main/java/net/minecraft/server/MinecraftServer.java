...
public abstract class MinecraftServer implements ICommandListener, Runnable, IAsyncTaskHandler, IMojangStatistics {
	...
	// private final MojangStatisticsGenerator m = new
	// MojangStatisticsGenerator("server", this, aw()); // CloudSpigot
	...
	public MinecraftServer(OptionSet options, Proxy proxy, DataConverterManager dataconvertermanager,
	...
	public void a(String s, String s1, long i, WorldType worldtype, String s2) {
		 ...
		 * 
		 * this.a(this.S(), idatamanager); WorldData worlddata =
		 * idatamanager.getWorldData(); WorldSettings worldsettings;
		 * 
		 * if (worlddata == null) { if (this.V()) { worldsettings = DemoWorldServer.a; }
		 * else { worldsettings = new WorldSettings(i, this.getGamemode(),
		 * this.getGenerateStructures(), this.isHardcore(), worldtype);
		 * worldsettings.setGeneratorSettings(s2); if (this.N) { worldsettings.a(); } }
		 * 
		...
		final java.util.Collection<String> toRemove = scoreboard.getTeams().stream()
				.filter(team -> team.getName().startsWith("collideRule_")).map(ScoreboardTeam::getName)
		...
		if (!com.destroystokyo.paper.PaperConfig.enablePlayerCollisions) {
			this.getPlayerList().collideRuleTeamName = org.apache.commons.lang3.StringUtils
					.left("collideRule_" + this.getWorld().random.nextInt(), 16);
		...
		for (int m = 0; m < worlds.size(); m++) {
			...
			for (int k = -radius; k <= radius && this.isRunning(); k += 16) {
				for (int l = -radius; l <= radius && this.isRunning(); l += 16) {
					...
					++i;
					worldserver.getChunkProviderServer().getChunkAt(blockposition.getX() + k >> 4,
		...
		if (file.isFile()) {
			try {
				this.setResourcePack(
						"level://" + URLEncoder.encode(s, StandardCharsets.UTF_8.toString()) + "/" + "resources.zip",
						"");
	...
	protected void saveChunks(boolean flag) {
		...
		for (int j = 0; j < worlds.size(); ++j) {
			...
			if (worldserver != null) {
				if (!flag) {
					MinecraftServer.LOGGER.info("Saving chunks for level \'{}\'/{}",
							worldserver.getWorldData().getName(), worldserver.worldProvider.getDimensionManager().b());
				}
	...
	public void stop() throws ExceptionWorldConflict { // CraftBukkit - added throws
		...
		if (this.worldServer != null) {
			...
			/*
			 * CraftBukkit start - Handled in saveChunks for (j = 0; j < i; ++j) {
			 * worldserver = aworldserver[j]; if (worldserver != null) {
		...
		try {
			if (this.init()) {
				...
				long start = System.nanoTime(), lastTick = start - TICK_TIME, catchupTime = 0, curTime, wait,
						tickSection = start; // Paper - Further improve server tick loop
			...
			File file = new File(new File(this.A(), "crash-reports"),
			...
			try {
				...
				try {
					net.minecrell.terminalconsole.TerminalConsoleAppender.close(); // Paper - Use
	...
	public void a(ServerPing serverping) {
		...
		if (file.isFile()) {
			...
			try {
				...
				serverping.setFavicon(
						"data:image/png;base64," + bytebuf1.toString(StandardCharsets.UTF_8).replace("\n", "")); // Paper
																													// -
																													// Fix
																													// encoding
																													// for
	...
	protected void C() throws ExceptionWorldConflict { // CraftBukkit - added throws
		...
		// this.methodProfiler.b();
		/*
		 * //this.methodProfiler.a("snooper"); if (getSnooperEnabled() && !this.m.d() &&
		 * this.ticks > 100) { // Spigot this.m.a(); }
		 * 
		 * if (getSnooperEnabled() && this.ticks % 6000 == 0) { // Spigot this.m.b(); }
		 * 
		 * //this.methodProfiler.b();
		 */ // CloudSpigot
		// this.methodProfiler.b();
	...
	public void D() {
		...
		for (i = 0; i < this.worlds.size(); ++i) { // CraftBukkit
			...
			TileEntityHopper.skipHopperEvents = worldserver.paperConfig.disableHopperMoveEvents
					|| org.bukkit.event.inventory.InventoryMoveItemEvent.getHandlerList()
							.getRegisteredListeners().length == 0;

			/*
			 * this.methodProfiler.a(() -> { return worldserver.getWorldData().getName();
			 * });
			 */
			/*
			 * Drop global time updates if (this.ticks % 20 == 0) {
			 * //this.methodProfiler.a("timeSync"); this.v.a((Packet) (new
			 * PacketPlayOutUpdateTime(worldserver.getTime(), worldserver.getDayTime(),
			 * worldserver.getGameRules().getBoolean("doDaylightCycle"))),
			 * worldserver.worldProvider.getDimensionManager().getDimensionID());
			 * //this.methodProfiler.b(); } // CraftBukkit end
			 */
	...
	public static void main(final OptionSet options) { // CraftBukkit - replaces main(String[] astring)
		...
		try {
			 ...
			 * 
			 * if (!"nogui".equals(s3) && !"--nogui".equals(s3)) { if ("--port".equals(s3)
			 * && s4 != null) { flag3 = true;
			 * 
			 * try { i = Integer.parseInt(s4); } catch (NumberFormatException
			 * numberformatexception) { ; } } else if ("--singleplayer".equals(s3) && s4 !=
			 * null) { flag3 = true; s = s4; } else if ("--universe".equals(s3) && s4 !=
			 * null) { flag3 = true; s1 = s4; } else if ("--world".equals(s3) && s4 != null)
			 * { flag3 = true; s2 = s4; } else if ("--demo".equals(s3)) { flag1 = true; }
			 * else if ("--bonusChest".equals(s3)) { flag2 = true; } } else { flag = false;
			 * }
			 * 
			 * if (flag3) { ++j; } }
			 */
			...
			String s1 = "."; // PAIL?
			YggdrasilAuthenticationService yggdrasilauthenticationservice = new com.destroystokyo.paper.profile.PaperAuthenticationService(
					Proxy.NO_PROXY, UUID.randomUUID().toString()); // Paper
			MinecraftSessionService minecraftsessionservice = yggdrasilauthenticationservice
					.createMinecraftSessionService();
			GameProfileRepository gameprofilerepository = yggdrasilauthenticationservice.createProfileRepository();
			UserCache usercache = new UserCache(gameprofilerepository, new File(s1, MinecraftServer.a.getName()));
			final DedicatedServer dedicatedserver = new DedicatedServer(options, DataConverterRegistry.a(),
					yggdrasilauthenticationservice, minecraftsessionservice, gameprofilerepository, usercache);

			/*
			 * CraftBukkit start if (s != null) { dedicatedserver.i(s); }
			 * 
			 ...
			 * 
			 * if (flag && !GraphicsEnvironment.isHeadless()) { dedicatedserver.aR(); }
			 * 
			 * dedicatedserver.F(); Runtime.getRuntime().addShutdownHook(new
	...
	public String getServerModName() {
		return "CloudSpigot"; // CloudSpigot - CloudSpigot > //Paper - Paper > // Spigot - Spigot > //
	...
	public List<String> tabCompleteCommand(ICommandListener icommandlistener, String s,
			@Nullable BlockPosition blockposition, boolean flag) {
		/*
		 * CraftBukkit start - Allow tab-completion of Bukkit commands ArrayList
		 * arraylist = Lists.newArrayList(); boolean flag1 = s.startsWith("/");
		 * 
		 * if (flag1) { s = s.substring(1); }
		 * 
		 * if (!flag1 && !flag) { String[] astring = s.split(" ", -1); String s1 =
		 * astring[astring.length - 1]; String[] astring1 = this.v.f(); int i =
		 * astring1.length;
		 * 
		 * for (int j = 0; j < i; ++j) { String s2 = astring1[j];
		 * 
		 * if (CommandAbstract.a(s1, s2)) { arraylist.add(s2); } }
		 * 
		 * return arraylist; } else { boolean flag2 = !s.contains(" "); List list =
		 * this.b.a(icommandlistener, s, blockposition);
		 * 
		 * if (!list.isEmpty()) { Iterator iterator = list.iterator();
		 * 
		 * while (iterator.hasNext()) { String s3 = (String) iterator.next();
		 * 
		 * if (flag2 && !flag) { arraylist.add("/" + s3); } else { arraylist.add(s3); }
		 * } }
		 * 
		 * return arraylist; }
		 */
	...
	public void sendMessage(IChatBaseComponent ichatbasecomponent) {
		...
		MinecraftServer.LOGGER.info(org.bukkit.craftbukkit.util.CraftChatMessage.fromComponent(ichatbasecomponent,
				net.minecraft.server.EnumChatFormat.WHITE));
	}
	...
	public void a(MojangStatisticsGenerator mojangstatisticsgenerator) {
		 ...
		 * Integer.valueOf(this.H())); mojangstatisticsgenerator.a("players_max",
		 * Integer.valueOf(this.I())); mojangstatisticsgenerator.a("players_seen",
		 * Integer.valueOf(this.v.getSeenPlayers().length)); }
		 * 
		 * mojangstatisticsgenerator.a("uses_auth", Boolean.valueOf(this.onlineMode));
		 * mojangstatisticsgenerator.a("gui_state", this.ap() ? "enabled" : "disabled");
		 * mojangstatisticsgenerator.a("run_time", Long.valueOf((aw() -
		 ...
		 * Integer.valueOf(worldserver.worldProvider.getDimensionManager().
		 * getDimensionID())); mojangstatisticsgenerator.a("world[" + i + "][mode]",
		 ...
		 * Boolean.valueOf(worlddata.isHardcore()));
		 * mojangstatisticsgenerator.a("world[" + i + "][generator_name]",
		 * worlddata.getType().name()); mojangstatisticsgenerator.a("world[" + i +
		 * "][generator_version]", Integer.valueOf(worlddata.getType().getVersion()));
		 * mojangstatisticsgenerator.a("world[" + i + "][height]",
		 * Integer.valueOf(this.G)); mojangstatisticsgenerator.a("world[" + i +
		 * "][chunks_loaded]",
		 * Integer.valueOf(worldserver.getChunkProviderServer().g())); ++i; } } }
		 * 
